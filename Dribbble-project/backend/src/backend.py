...
class Backend:
    def __init__(self, app_secret_key, files_path, app_base_url) -> None:
        ...
        self.category_manager = CategoryManager(self.database_session)
    ...
    def add_mock_values_to_db(self):
        ...
        self.database_session.add(Seller(name='baran', username='user0', password=generate_password_hash('password'), is_active=True))
        self.database_session.add(
            Customer(name='baran', credit=100000000.0, join_date=datetime.datetime.now(), is_active=True, phone_number='09101010203'))
        self.database_session.add(Payer(name='baran-payer', phone_number='09111010203'))
        self.database_session.add(Category(name='طرح'))
        self.database_session.add(Product(name='Lock screen Massage', price=2000000, stock_number=5, category_id=1, is_active=True))
        self.database_session.add(Product(name='Ui Ux Desdign', price=3500000, stock_number=2, category_id=1, is_active=True))
        self.database_session.add(Product(name='M logo Design', price=80000, stock_number=10, category_id=1, is_active=True))
        self.database_session.add(Product(name='Creat a website the way you want', price=19000000, stock_number=10, category_id=1, is_active=True))
        self.database_session.add(Product(name='Platform Ui Design', price=5800000, stock_number=1, category_id=1, is_active=True))
        self.database_session.commit()
